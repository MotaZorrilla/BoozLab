...
export function useWhatsApp() {
    const { props } = usePage();
    const settings =
        ((props as Record<string, unknown>).settings as Record<
            string,
            string
        >) || {};
    ...
    const defaultMessage =
        ...
        'Hola Booz Laboratorio, deseo realizar una consulta.';
    const cartHeader =
    ...
    const cleanSalesPhone = useMemo(
        () => salesPhone.replace(/\D/g, ''),
        [salesPhone],
    );
    const cleanContactPhone = useMemo(
        () => contactPhone.replace(/\D/g, ''),
        [contactPhone],
    );
    const cleanCompanyPhone = useMemo(
        () => companyPhone.replace(/\D/g, ''),
        [companyPhone],
    );

    const customerTypes = useMemo(() => {
        return rawCustomerTypes
            .split(',')
            .map((s: string) => s.trim())
            .filter(Boolean);
    }, [rawCustomerTypes]);
    ...
    const createWhatsAppUrl = (
        customText?: string,
        targetPhoneOrChannel?: 'contact' | 'sales' | string,
    ) => {
